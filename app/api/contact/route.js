...
import nodemailer from "nodemailer"; // Make sure to npm install nodemailer

export async function POST(req) {
  try {
    await dbConnect();
    const body = await req.json();
    const { name, email, phone, service, projectVision } = body;

    // 1. Save to MongoDB
    const newInquiry = await Inquiry.create({
      name, email, phone, service, projectVision,
    });
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: process.env.EMAIL_USER, // Your email (e.g., hello@tarunainterior.com)
    ...
    html: `
        ...
        <div style="max-width: 600px; mx-auto; background-color: #111111; border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.4);">
            ...
            <div style="margin-bottom: 30px;">
                ...
                <p style="margin: 0 0 15px 0; font-size: 16px;"><strong style="color: #4318FF; width: 80px; display: inline-block;">Phone:</strong> <span style="color: #ffffff;">${phone}</span></p>
                <p style="margin: 0; font-size: 16px;"><strong style="color: #4318FF; width: 80px; display: inline-block;">Service:</strong> <span style="color: #ffffff; text-transform: capitalize;">${service}</span></p>
                </div>
            </div>
