  ...
  return (
    <main className="relative bg-[#0a0a0a] min-h-screen pt-24 pb-32 text-white selection:bg-[#232a8b] selection:text-black overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <motion.div 
          initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.5, ease: customEase }}
          className="relative w-full h-[60vh] md:h-[80vh] rounded-[2rem] overflow-hidden mb-24 bg-[#050505]"
        >
          <Image 
            src={project.mainImage} alt={project.title} fill priority
        ...
        {project.gallery && project.gallery.length > 0 && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
              {project.gallery.slice(0, 4).map((imgUrl, index) => (
                <motion.div 
                  key={index} 
                  ...
                  className={`relative w-full rounded-[2rem] overflow-hidden bg-[#050505] group h-[400px] lg:h-[600px]`}
                >
                  <Image 
                    src={imgUrl} alt={`Detail ${index + 1}`} fill
            ...
            <div className="mt-16 flex justify-center">
              <Link 
                href={`/portfolio/${project.id}/fullportfolio`}
                className="group flex items-center gap-3 border border-white/20 px-8 py-4 rounded-full hover:bg-white hover:text-black transition-all duration-500"
              >
                <span className="text-sm font-semibold uppercase tracking-widest">View Full Gallery</span>
